...
---
롤링 백테스트 엑셀 리포트 데이터에서 차트를 만들고 싶어. 
인사이트를 얻을 수 있는 차트는 어떤게 있을까? 제안해줘.

---

아래처럼 수정해줘.

너는 유능한 개인 자산관리사라고 생각하고 일시 VS 적립 투자를 분석해줘.

큰 범위인 롤링 분석을 앞에다 두고 개별분석을 뒤에다 배치해줘.
차트는 굳이 다 쓸 필요는 없어. 너가 필요하다고 판단하는 차트만 써도 되.
분석에 결과인 엑셀파일 링크를 걸어줘.
그리고 아래 내용에 대해서 다시 생각해줘. 시장 고점, 저점을 투자 당시에는 판단할 수 없잖아. 의미 없는 글이야.
