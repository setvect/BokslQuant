...
class LumpSumVsDcaAnalyzer:
    ...
    def export_detailed_analysis_to_excel(self, records: List[MonthlyRecord], 
        ...
        # MDD 누적 최대 수익률 계산 및 업데이트  
        self._calculate_mdd_peak_returns(df)
        
        # 정확한 MDD 재계산 (수익률 기반)
        self._recalculate_mdd_based_on_returns(df)
        ...
        # 데이터 추가 (DataFrame에서 직접 가져오기)
        for row_idx in range(len(df)):
            for col_idx, header in enumerate(headers, 1):
                value = df.iloc[row_idx][header]
                cell = ws_detail.cell(row=row_idx + 2, column=col_idx, value=value)
                # 정렬은 나중에 일괄 적용
    ...
    def _calculate_mdd_peak_returns(self, df: pd.DataFrame) -> None:
        """
        MDD 누적 최대 수익률 계산 및 DataFrame 업데이트
        
        Args:
            df: 월별 상세 데이터 DataFrame
        """
        # 일시금 누적 최대 수익률 계산
        lump_sum_peak_return = 0.0
        for i in range(len(df)):
            current_return = df.iloc[i]['일시금_누적수익률']  # 이미 소수 형태 (0.0358 = 3.58%)
            lump_sum_peak_return = max(lump_sum_peak_return, current_return)
            df.iloc[i, df.columns.get_loc('일시금_누적_최대_수익률')] = lump_sum_peak_return
        
        # 적립식 누적 최대 수익률 계산
        dca_peak_return = 0.0
        for i in range(len(df)):
            current_return = df.iloc[i]['적립식_누적수익률']  # 이미 소수 형태
            if current_return != 0:  # 투자가 시작된 후부터
                dca_peak_return = max(dca_peak_return, current_return)
            df.iloc[i, df.columns.get_loc('적립식_누적_최대_수익률')] = dca_peak_return
    
    def _recalculate_mdd_based_on_returns(self, df: pd.DataFrame) -> None:
        """
        수익률 기반으로 MDD 재계산
        ...
        # 일시금 MDD 재계산
        for i in range(len(df)):
            current_return = df.iloc[i]['일시금_누적수익률']
            peak_return = df.iloc[i]['일시금_누적_최대_수익률']
            new_mdd = current_return - peak_return
            df.iloc[i, df.columns.get_loc('일시금_MDD')] = new_mdd
        
        # 적립식 MDD도 동일하게 재계산
        for i in range(len(df)):
            current_return = df.iloc[i]['적립식_누적수익률']
            peak_return = df.iloc[i]['적립식_누적_최대_수익률']
            new_mdd = current_return - peak_return
            df.iloc[i, df.columns.get_loc('적립식_MDD')] = new_mdd
    ...
    def calculate_daily_mdd(self, price_data: pd.DataFrame, 
        ...
        # 일시투자 수익률 계산용 변수
        lump_sum_cumulative_max_return = 0.0
        
        # 적립투자 계산용 변수  
        dca_total_shares = 0
        dca_total_invested = 0
        dca_cumulative_max_return = 0.0
        ...
        for i, (date, row) in enumerate(period_data.iterrows()):
            ...
            # === 일시투자 MDD 계산 (수익률 기반) ===
            # Close 가격 기준 수익률 계산
            lump_sum_close_return = (close_price / initial_price - 1) * 100
            # Low 가격 기준 수익률 계산 (일중 최저점)
            lump_sum_low_return = (low_price / initial_price - 1) * 100
            
            # 누적 최대 수익률 업데이트 (Close 기준)
            lump_sum_cumulative_max_return = max(lump_sum_cumulative_max_return, lump_sum_close_return)
            
            # MDD 계산 (Low 기준으로 최대 낙폭 계산)
            lump_sum_mdd = min(0, lump_sum_low_return - lump_sum_cumulative_max_return)
            ...
            # 적립투자 수익률 계산
            if dca_total_shares > 0 and dca_total_invested > 0:
                # Close 가격 기준 수익률
                dca_close_value = dca_total_shares * close_price
                dca_close_return = (dca_close_value / dca_total_invested - 1) * 100
                
                # Low 가격 기준 수익률 (일중 최저점)
                dca_low_value = dca_total_shares * low_price
                dca_low_return = (dca_low_value / dca_total_invested - 1) * 100
                
                # 누적 최대 수익률 업데이트 (Close 기준)
                dca_cumulative_max_return = max(dca_cumulative_max_return, dca_close_return)
                
                # MDD 계산 (Low 기준으로 최대 낙폭 계산)
                dca_mdd = min(0, dca_low_return - dca_cumulative_max_return)
            else:
                dca_mdd = 0.0
    ...
    def create_detailed_charts(self, records: List[MonthlyRecord], 
        ...
        Args:
            records: 월별 기록 리스트
            start_date: 투자 시작일
            price_data: 가격 데이터 (사용하지 않음, 월별 데이터 기반으로 계산)
        ...
        # 일별 MDD 계산 (정확한 MDD 측정)
        start_datetime = datetime.strptime(start_date, '%Y-%m-%d')
        end_datetime = start_datetime + pd.DateOffset(years=self.config.analysis_period_years)
        ...
        if daily_dates and daily_lump_sum_mdds and daily_dca_mdds:
            ...
            # 일별 데이터가 없는 경우 빈 차트 생성
            plt.figure(figsize=(16, 10))
            plt.text(0.5, 0.5, '일별 MDD 데이터를 계산할 수 없습니다.', 
